...
/*
      ...
      int cluster = print_sub(to_string(&b),true)
        //print branchBlock internal tree
      print_sub("BranchBlock",false);

      for (auto &targetb : f) {
        if (DT.dominates(&b,&targetb)) {
          addNode(targetb);
          addEdge(b,targetb,"dom");
      }
...
Vvoid
moveControlEdge(Value &control, Value &target, bool recurse) {
  bool dupEdge = false;
 

  auto controlNode = node_map.find(control);
  if (controlNode == node_map.end()) { outs() << "Shouldn't happen \n"; return; }

  auto controlEdges = controlNode->second;

  auto targetNode = node_map.find(&target);
  if (targetNode == node_map.end()) { outs() << "Shouldn't happen either \n"; return; }

  auto targetEdges = targetNode->second;


  for (auto otherControl : controlEdges) {
    Value *otherControlNode = otherControl.first;
    if (targetEdges.find(otherControlNode) != targetEdges.end()) {
      addEdge(target,*otherControlNode,"control");
      //delEdge(control,otherControlNode)
    }
    if (recurse) moveControlEdge(control,*otherControlNode,false);
  }



  for (auto C : targetEdges) {
      Value *nodeC = C.first;

      if (controlEdges.find(nodeC) != controlEdges.end()) {
        
        outs() << "Duplicate edge between \n"
              << " - Control: " << control << "\n"
              << " - Target: " << target << "\n"
              << " - Duplicate: " << *nodeC << "\n";
      }     
  ...
  for (User *U : target.users()) {
    if (controlEdges.find(dyn_cast<Value>(U)) != controlEdges.end()) {
//      outs() << "Duplicate edge between \n"
//             << " - Control: " << control << "\n"
//             << " - Target: " << target << "\n"
//             << " - Duplicate: " << *U << "\n";
      addEdge(target,*U,"control");
      dupEdge = true;
    }
  }
  if (!dupEdge)  addEdge(control,target,"control" + to_string(num));
  */
